...
namespace TP11___Web_Cursos.Models
{
    public class BD
    {
        private static string _connectionString = @"Server=GUSABELU1\SQLEXPRESS;DataBase=BDPadron;Trusted_Connection=True;";
        ...
        public static Curso ListarCursos(int IdEspecialidad){
            Curso TempCurso = null;
            if(IdEspecialidad == -1){
                string sql = "SELECT * FROM Cursos";
                using(SqlConnection db = new SqlConnection(_connectionString)){
                    TempCurso = db.QueryFirstOrDefault<Curso>(sql);
                }   
            }
            else{
                string sql = "SELECT * FROM Cursos WHERE IdEspecialidad = @pEspecialidad";
                using(SqlConnection db = new SqlConnection(_connectionString)){
                    TempCurso = db.QueryFirstOrDefault<Curso>(sql, new{ pEspecialidad = IdEspecialidad });
                }
            }
            return TempCurso;
        }
